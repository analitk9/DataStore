    ...
    func fetchPosts(_ hangler: @escaping (Result<[Post], PostError>)->Void ){
     ...
     func createMockData()-> [Post]?{
       ...
       posts.append(Post(title: "Физики обнаружили новый тип сверхпроводимости и ранее неизвестное состояние вещества", author: "3DNews", description: "Международная группа физиков из Германии и Швейцарии экспериментально обнаружила и подтвердила существование неизвестного ранее состояния вещества, сопутствующего эффекту сверхпроводимости. Явление оказалось настолько уникальным, что предсказать последствия открытия сегодня очень сложно, хотя оно вселяет большие надежды на прорыв в области сверхпроводимости и квантовых приборов. Это первый практический шаг в новую область знаний.", image: "news2", likes: 0, views: 120))
        ...
        posts.append(Post(title: "Из полиэтиленовых пакетов научились готовить дизельное топливо", author: "Ferra", description: "Исследователи изобрели новый метод, который позволяет переработать пластиковые пакеты в дизельное топливо. В основу методики лёг каталитический пиролиз — термохимическое разложение вещества на основе углерода.", image: "news4", likes: 101, views: 56))
